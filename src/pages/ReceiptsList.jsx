...
const ReceiptsList = () => {
  ...
  const handleCancelConfirm = async () => {
    ...
    try {
      ...
      setSuccess(`Receipt ${selectedReceipt.transaction_no} has been cancelled successfully`);
  ...
  return (
    <Box p={3} maxWidth="800px" mx="auto">
            ...
            <Accordion key={regNo} defaultExpanded sx={{ mb: 2 }}>
              <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                <Box display="flex" width="100%" justifyContent="space-between" alignItems="center">
                  ...
                  <Typography>
                    Patient: {regReceipts[0]?.patient_name} (UHID: {regReceipts[0]?.patient_uhid})
                  </Typography>
                  <Typography>
                    {regReceipts.length} Receipt(s)
                  </Typography>
                </Box>
              </AccordionSummary>
              <AccordionDetails>
                        ...
                        <CardContent>
                          <Box display="flex" flexWrap="wrap" gap={2}>
                            <Box flex="1 1 30%">
                              ...
                              <Typography>{receipt.transaction_no}</Typography>
                            </Box>
                            
                            <Box flex="1 1 30%">
                              <Typography fontWeight="bold">Purpose:</Typography>
                              <Typography>{receipt.transaction_purpose}</Typography>
                            </Box>
                            
                            <Box flex="1 1 30%">
                              <Typography fontWeight="bold">Date:</Typography>
                              <Typography>{receipt.transaction_date}</Typography>
                            </Box>
                            
                            <Box flex="1 1 30%">
                              <Typography fontWeight="bold">Time:</Typography>
                              <Typography>{receipt.transaction_time}</Typography>
                            </Box>
                            ...
                            {receipt.payment_mode === "CHEQUE" && receipt.payment_details && (
                              <>
                                <Box flex="1 1 30%">
                                  <Typography fontWeight="bold">Bank:</Typography>
                                  <Typography>{receipt.payment_details.bank_name}</Typography>
                                </Box>
                                <Box flex="1 1 30%">
                                  <Typography fontWeight="bold">Cheque No:</Typography>
                                  <Typography>{receipt.payment_details.cheque_number}</Typography>
                                </Box>
                                <Box flex="1 1 30%">
                                  <Typography fontWeight="bold">Cheque Date:</Typography>
                                  <Typography>{receipt.payment_details.cheque_date}</Typography>
                                </Box>
                              </>
                            )}
      ...
      <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)}>
        ...
        <DialogContent>
          <Typography>
            Are you sure you want to cancel receipt {selectedReceipt?.transaction_no}?
